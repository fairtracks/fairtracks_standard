...
jsonschema2md: $(JSONSCHEMA2MD_BIN_PATH) $(CLEANUP_DOCS_SCRIPT) $(VENV_ACTIVATE)
	. $(VENV_ACTIVATE)
	$(JSONSCHEMA2MD_BIN_PATH) --input $(SCHEMA_DIR) --out $(DOCS_DIR) -n -p format,ontology,ancestors,namespace,matchType,foreignProperty,unique,autogenerated,constIf
	rm $(DOCS_SCHEMA_FILES)
	python3 $(CLEANUP_DOCS_SCRIPT) $(DOCS_MARKDOWN_FILES)
