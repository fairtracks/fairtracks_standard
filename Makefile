...
json: $(SCHEMA_FILES) $(EXAMPLE_FILES) $(BLUEPRINT_DIR)/blueprint_minimal.json $(BLUEPRINT_DIR)/blueprint_augmented.json
...
jsonschema2md: $(JSONSCHEMA2MD_BIN_PATH) $(CLEANUP_DOCS_SCRIPT) $(GIT_HOOKS_FILES)
	rm -rf $(DOCS_DIR)/*.md
	. $(VENV_ACTIVATE); $(JSONSCHEMA2MD_BIN_PATH) --input $(SCHEMA_DIR) --out $(DOCS_DIR) -x - -n -p format ontology ancestors namespace matchType foreignProperty unique augmented
