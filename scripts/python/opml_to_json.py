...
ROOT_FILE_PATH = 'opml/fairtracks.overview.opml'
...
AUTOGENERATED_ATTRIB = 'autogenerated'
...
def main():
    ...
    if args.json_type == 'schema':
        ...
        if args.in_opml.name == ROOT_FILE_PATH:
            opml_elem = ElementTree.parse(args.in_opml.name).find('./body')
            augmented_attributes = _collect_augmented_attributes(opml_elem, args.in_opml.name)
            _json_schema_add_augmented_condition(json_dict, augmented_attributes)
...
def _json_schema_add_augmented_condition(json_dict, augmented_attributes):
    all_of = [{'if': {
        'properties': {
            'doc_info': {
                'properties': {
    ...
    then_obj = NestedOrderedDict()
    for path in augmented_attributes:
        nested_ordered_dict = then_obj
        for i, attribute in enumerate(path, start=1):
            if attribute == 'required':
                if 'required' not in nested_ordered_dict:
                    nested_ordered_dict['required'] = []
                continue
            if i == len(path):
                nested_ordered_dict['required'] += [attribute]
                continue
            nested_ordered_dict = nested_ordered_dict[attribute]
    all_of[0]['then'] = dict(then_obj)
    json_dict['allOf'] = all_of
    return json_dict
...
def _collect_augmented_attributes(opml_elem, opml_file_path):
    path = []
    paths = []
    _collect_augmented_attributes_recursive(opml_elem, opml_file_path, path, paths)
    return paths


def _collect_augmented_attributes_recursive(opml_elem, opml_file_path, path, paths):
    elem_name = _get_opml_el_name(opml_elem)
    if elem_name:
        path += ['required' if _is_augmented(opml_elem) else 'properties', elem_name]
        if 'type' in opml_elem.attrib and opml_elem.attrib['type'] == 'array':
            path += ['items']
    if _is_augmented(opml_elem):
        paths += [path]
    for opml_child in opml_elem:
        local_path = path.copy()
        if _is_ref(opml_child) and opml_child.attrib['ref'] != '.':
            child_name = _get_opml_el_name(opml_child)
            if child_name:
                local_path += ['properties', child_name]
            ref_opml_file_path = _generate_opml_file_path_from_ref(opml_file_path, opml_child)
            opml_child = ElementTree.parse(ref_opml_file_path).find('./body')
        _collect_augmented_attributes_recursive(opml_child, opml_file_path, local_path, paths)


def _is_augmented(opml_elem):
    return AUTOGENERATED_ATTRIB in opml_elem.attrib and opml_elem.attrib[AUTOGENERATED_ATTRIB] == 'true'
...
def _json_schema_create_root(opml_root):
    ...
    version_url = opml_root.find(".//outline[@_name='#version_url']")
